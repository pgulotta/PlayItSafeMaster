...
void PdfBuilder::run() {
  try {
    ...
    printer.setPageMargins(40, 40, 0, 40, QPrinter::DevicePixel);
    ...
    cursor.insertText(QDate::currentDate().toString(Qt::SystemLocaleLongDate));

    appendTitleRow(mSwitchboardManager.getTitleFromMoniker(SwitchboardCategory::Moniker::Recap), calculateTotal(mRecapListModel), cursor);
    cursor.insertText( Common::NewLine );
    appendTextRows(mRecapListModel, cursor);

    appendTitleRow(mSwitchboardManager.getTitleFromMoniker(SwitchboardCategory::Moniker::BankAccount), calculateTotal(mBankAccountListModel),
                   cursor);
    cursor.insertText( Common::NewLine );
    appendTextRows(mBankAccountListModel, cursor);

    appendTitleRow(mSwitchboardManager.getTitleFromMoniker(SwitchboardCategory::Moniker::Investment), calculateTotal(mInvestmentListModel),
                   cursor);
    cursor.insertText( Common::NewLine );
    appendTextRows(mInvestmentListModel, cursor);

    appendTitleRow(mSwitchboardManager.getTitleFromMoniker(SwitchboardCategory::Moniker::RealAsset), calculateTotal(mRealAssetListModel),
                   cursor);
    cursor.insertText( Common::NewLine );
    appendTextRows(mRealAssetListModel, cursor);

    appendTitleRow(mSwitchboardManager.getTitleFromMoniker(SwitchboardCategory::Moniker::Expense), calculateTotal(mExpenseListModel), cursor);
    cursor.insertText( Common::NewLine );
