...
bool DownloadsPathController::setDownloadsPath(const QString& downloadsPath) {
  ...
  try {
    ...
    qInfo() << "DownloadsPathController::setDownloadsPath path = " + settings.value(Common::DownloadsPathSettingsKey).toString();
  } catch (const std::exception& e) {
    qWarning() << Q_FUNC_INFO << " " << e.what();
  }
  return isSuccessful;
...
QString DownloadsPathController::initAppDownloadFolder() {
  ...
  try {
    ...
    qInfo() << "DownloadsPathController::initAppDownloadFolder path = " + settings.value(Common::DownloadsPathSettingsKey).toString();
  } catch (const std::exception& e) {
    qWarning() << Q_FUNC_INFO << " " << e.what();
  }
  return downloadsPath;
...
QString DownloadsPathController::tryResolveFilDownloadePath(const QString& unresolvedPath) {
  ...
  if (QFile::exists(unresolvedPath)) {
    ...
    QString fileName = DownloadsPathController::downloadsPath() + "/" +  fileInfo.fileName();
