 ...
 // qInfo()  <<  "FileEncryptor::decrypt: mEncryptedFileName=" << qPrintable(mDataStoreFileNames.encryptedFileName());
  ...
  try {
    if (mEncryptedFilePassword.isEmpty()) {
      ...
      qWarning()  << "FileEncryptor::decrypt:  Encrypted file does not exist: " << qPrintable(mDataStoreFileNames.encryptedFileName());
    } else if ( QFileInfo(mDataStoreFileNames.encryptedFileName()).size() == 0 ) {
      qWarning()  << "FileEncryptor::decrypt:  Encrypted file size is zero: " << qPrintable(mDataStoreFileNames.encryptedFileName());
    } else {
    ...
    qWarning("FileEncryptor::decrypt: Failed to write file %s; %s", qPrintable(mDataStoreFileNames.encryptedFileName()), e.what());
  }
  return result;
}
...
bool FileEncryptor::encrypt() {
  ...
  try {
    ...
    qWarning("FileEncryptor::encrypt: Failed to write file %s; %s", qPrintable(mDataStoreFileNames.encryptedFileName()), e.what());
  }
