...
void TableSelect::select( ConnectionPtr& connection, Version* version )
{
  for ( Row row : Statement( *connection.data(), "SELECT * FROM Version" ) ) {
    version->setCreatedDate( QDateTime::fromSecsSinceEpoch( static_cast<uint>( row.GetInt( 0 ) ) ) );
    version->setLastUsedDate( QDateTime::fromSecsSinceEpoch( static_cast<uint>( row.GetInt( 1 ) ) ) );
    version->setReleaseNumber( row.GetInt( 2 ) );
  }

  qInfo() << "DataStoreManager::version:   createdDate:" << version->createdDate() << "  lastUsedDate:" <<
          version->lastUsedDate() << "  releaseNumber:" << version->releaseNumber();
}
...
void TableSelect::select( ConnectionPtr& connection, QQmlObjectListModel<Expense>& list )
{
  ...
  for ( Row row : Statement( *connection.data(),
                             "SELECT E.*, W.Url FROM Expense E LEFT OUTER JOIN Website W on  E.WebsiteId = W.UniqueId ORDER BY E.Payee" ) ) {
    int index = 0;
...
void TableSelect::select( ConnectionPtr& connection, QQmlObjectListModel<RealAsset>& list )
{
  ...
  for ( Row row : Statement( *connection.data(),
                             "SELECT R.* , W.Url FROM RealAsset R LEFT OUTER JOIN Website W on R.WebsiteId = W.UniqueId ORDER BY Description" ) ) {
    int index = 0;
...
void TableSelect::select( ConnectionPtr& connection, QQmlObjectListModel<BankAccount>& list )
{
  ...
  for ( Row row : Statement( *connection.data(),
                             "select BA.*, W.Url FROM BankAccount BA LEFT OUTER JOIN Website W on  BA.WebsiteId = W.UniqueId ORDER BY BA.BankName" ) ) {
    int index = 0;
...
void TableSelect::select( ConnectionPtr& connection, QQmlObjectListModel<Investment>& list )
{
  ...
  for ( Row row : Statement( *connection.data(),
                             "SELECT I.*, W.Url FROM Investment I LEFT OUTER JOIN Website W on I.WebsiteId = W.UniqueId  ORDER BY Issuer" ) ) {
    int index = 0;
